...
<div class="row">
	...
	<div class="col-md-9 tit_ric">
		<div class="col-md-12 form-group">
			<h4><?php 
				if( sizeof($res) > 1 ) {
					echo $this->Paginator->counter(__('Trovati %s passaggi per la tratta da %s a %s', array('{:count}', $this->App->stripNazione($criteria['from']['fulladdress']), $this->App->stripNazione($criteria['to']['fulladdress']))));
				}
				else {
					echo $this->Paginator->counter(__('Trovato 1 passaggio per la tratta da %s a %s', array('{:count}', $this->App->stripNazione($criteria['from']['fulladdress']), $this->App->stripNazione($criteria['to']['fulladdress']))));
				}
				if( !empty($criteria['outward']['mindate']) || !empty($criteria['outward']['maxdate']) ) echo '<br/><br/>';
				if( !empty($criteria['outward']['mindate']) ) echo __('a partire dal %s', array(date('d-m-Y', strtotime($criteria['outward']['mindate'])))).' ';
				if( !empty($criteria['outward']['maxdate']) ) echo __('fino al %s', array(date('d-m-Y', strtotime($criteria['outward']['maxdate']))));
			?></h4>
		</div>    
		<div class="col-md-12 form-group">
			<?php if($layout == 'embed'):?>
...
<div class="row">
	...
	<div class="col-md-9 tit_ric">
		<div class="col-md-12 form-group">
			<h4><?php
				echo __('Nessun risultato trovato per la tratta da %s a %s', array($this->App->stripNazione($criteria['from']['fulladdress']), $this->App->stripNazione($criteria['to']['fulladdress'])));
				if( !empty($criteria['outward']['mindate']) || !empty($criteria['outward']['maxdate']) ) echo '<br/><br/>';
				if( !empty($criteria['outward']['mindate']) ) echo __('a partire dal %s', array(date('d-m-Y', strtotime($criteria['outward']['mindate'])))).' ';
				if( !empty($criteria['outward']['maxdate']) ) echo __('fino al %s', array(date('d-m-Y', strtotime($criteria['outward']['maxdate']))));
			?></h4>
...
<div id="search-filters">	
	<?php 
		...
		echo $this->Form->create('Journey', array('id' => 'RicalcolaForm', 'action' => 'search?layout='.$layout));
	?>
...
<div class="row tit_ric">
	<div class="col-md-6 form-group">
		<p><?php echo $this->Paginator->counter(__('Pagina %s di %s, %s risultati da %s a %s', array('{:page}', '{:pages}', '{:current}', '{:start}', '{:end}')));?></p>
	</div>
			...
			<div class="btn-group" role="group" aria-label="...">
				<?php 
					if(!isset($this->request->params['named']['sort'])) {
						...
						$returnCls = 'btn-default';
						$priceCls = 'btn-default';
						switch($this->request->params['named']['sort']) {
							case 'departure':
								$departureCls = 'btn-primary';
								break;
							case 'return_maxdate':
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu" role="menu">
						<li><?php echo $this->Html->link('<span class="glyphicon glyphicon-sort-by-attributes" aria-hidden="true"></span> '.__('crescente'), array('action' => 'search_result', 'sort:departure', 'direction:asc', '?' => array('layout' => $layout)), array('escape' => false));?></li>
						<li><?php echo $this->Html->link('<span class="glyphicon glyphicon-sort-by-attributes-alt" aria-hidden="true"></span> '.__('decrescente'), array('action' => 'search_result', 'sort:departure', 'direction:desc', '?' => array('layout' => $layout)), array('escape' => false));?></li>
					</ul>
				</div>
				<!--div class="btn-group">
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu pull-right" role="menu">
						<li><?php echo $this->Html->link('<span class="glyphicon glyphicon-sort-by-attributes" aria-hidden="true"></span> '.__('crescente'), array('action' => 'search_result', 'sort:cost_fixed', 'direction:asc', '?' => array('layout' => $layout)), array('escape' => false));?></li>
						<li><?php echo $this->Html->link('<span class="glyphicon glyphicon-sort-by-attributes-alt" aria-hidden="true"></span> '.__('decrescente'), array('action' => 'search_result', 'sort:cost_fixed', 'direction:desc', '?' => array('layout' => $layout)), array('escape' => false));?></li>
					</ul>
				</div>
			</div>
	</div>
